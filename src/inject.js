(function () {
  ...
  Object.assign(bar.style, {
    ...
    padding:'0 10px', background:'rgba(13,27,42,.97)',
    borderBottom:'1px solid rgba(30,144,255,.22)',
  ...
  function mkBtn(iconHtml, title, fn) {
    ...
    b.onmouseenter = () => { b.style.background='rgba(30,144,255,.12)'; b.style.color=C1; };
  ...
  Object.assign(logo.style, {
    fontSize:'14px', fontWeight:'900', letterSpacing:'3px', color:C1,
  ...
  Object.assign(urlWrap.style, {
    ...
    background:'#1a2f4a', border:'1px solid rgba(30,144,255,.22)',
  ...
  urlIn.addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      e.preventDefault();
      window.pywebview?.api?.navigate(urlIn.value.trim());
    }
    if (e.key === 'Escape') urlIn.blur();
  });
  ...
  document.body.style.cssText += `;margin-top:${BAR_H}px!important`;
  ...
  (async () => {
    try {
      await window.pywebview?.api?.add_history(location.href, document.title || location.href);
    } catch { /* silent */ }
  })();
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.ctrlKey && e.key === 'l')          { e.preventDefault(); urlIn.select(); urlIn.focus(); }
  });
